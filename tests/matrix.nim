...
let
    ...
    e: Mat3 = [[2, 1, 0],
               [4, 3, 2],
               [1, 0, 1]]
    f = tform(vec(1, 0), vec(2, 1), vec(2, -1))
    g = tform(vec(1, 0, 0), vec(0, 1, -1), vec(-1, 0, 1), vec(-1, -1, 1))
...
test "Basics":
    check:
        ...
        (2 * sizeof Vec3) == sizeof Transform2D
        (3 * sizeof Vec4) == sizeof Transform3D
    ...
    let `2 * f`: Transform2D = [[2, 0], [4, 2], [4, -2]]
    check:
        `3 * c` =~ 3*c
        `2 * f` == 2*f
